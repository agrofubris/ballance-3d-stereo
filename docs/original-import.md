...
BMap's PNG export writes every texture without alpha (`StbSaveFile` is called with `save_alpha=false`), so `prepare-original.py` re-extracts the alpha-bearing TGAs that the runtime samples from the game's `Textures` directory with ffmpeg: the dome and transformer shadow decals, `Laterne_Schatten`, `Modul18_Gitter`, `Stick_Bottom` and `Column_beige_fade`. `Laterne_Verlauf` is deliberately excluded because its source alpha is uniformly zero; restoring it would erase the lamp columns instead of preserving them. The menu button textures and `Font_1` also lose alpha in conversion, but the current interface renders browser text and CSS frames rather than those bitmaps, so they stay on the default path.
