...
native_compiler = shutil.which('clang++') or shutil.which('c++')
if native_compiler is None:
    raise RuntimeError('A native C++ compiler (clang++ or c++) is required for the IVP verification bridge')

def link_libraries(build: Path):
    """Return static archives with GNU rescan grouping on Linux.

    IVP has mutually-referencing static libraries. GNU ld only makes a single
    left-to-right pass unless the archives are wrapped in a linker group;
    Apple ld and the original reference environment do not need these flags.
    Emscripten uses the same GNU-style linker semantics when invoked on Linux.
    """
    archives = [str(build / ('lib' + lib + '.a')) for lib in libraries]
    return ['-Wl,--start-group', *archives, '-Wl,--end-group'] if platform.system() == 'Linux' else archives

def patch_linux_polygon_header(build: Path):
    """Give GCC a key virtual function so IVP_Polygon's vtable is emitted.

    The legacy header uses GCC's obsolete ``#pragma interface`` and declares an
    empty subclass of IVP_Real_Object. Modern GCC consequently does not emit
    that subclass vtable in a static archive, while the native and Emscripten
    linkers still require it. Patch only CMake's generated include copy; the
    pinned SDK checkout remains untouched and byte-verifiable.
    """
    if platform.system() != 'Linux':
        return
    header = build / 'include/ivp/ivp_polygon.hxx'
    if not header.exists():
        return
    source_text = header.read_text()
    patched = source_text.replace('#ifndef WIN32\n#pragma interface\n#endif\n', '')
    patched = patched.replace('public:\n};', 'public:\n    virtual ~IVP_Polygon() = default;\n};')
    if patched == source_text:
        raise RuntimeError('Could not patch generated IVP polygon header')
    header.write_text(patched)

def patch_linux_polygon_source():
    """Temporarily patch the source header before CMake stages public headers."""
    if platform.system() != 'Linux':
        return
    header = source / 'ivp_physics/ivp_polygon.hxx'
    original = header.read_bytes()
    source_text = header.read_text()
    patched = source_text.replace('#ifndef WIN32\n#pragma interface\n#endif\n', '')
    patched = patched.replace('public:\n};', 'public:\n    virtual ~IVP_Polygon() = default;\n};')
    if patched == source_text:
        raise RuntimeError('Could not patch IVP polygon source header')
    header.write_text(patched)
    # The downloaded checkout must remain byte-identical after this process.
    atexit.register(lambda: header.write_bytes(original))
...
for target in ['native', 'wasm']:
    ...
    native_cmake_flags = ['-DCMAKE_CXX_COMPILER=' + native_compiler] if target == 'native' else []
    run((['emcmake'] if target == 'wasm' else []) + ['cmake', '-S', str(source), '-B', str(build), *native_cmake_flags,
    ...
    patch_linux_polygon_header(build)
    ...
    run(['em++' if target == 'wasm' else native_compiler, '-std=c++17', '-O2', '-DUNIX', '-DIVP_VERSION_SDK',
         '-fno-strict-aliasing', '-ffp-contract=off', *native_matrix_flags, '-I' + str(build / 'include/ivp'), str(wrapper),
         *link_libraries(build), *flags,
    ...
    if target == 'native':
        ...
        run([native_compiler, '-std=c++17', '-O2', '-DUNIX', '-DIVP_VERSION_SDK',
