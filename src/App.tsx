...
export default function App() {
  ...
  return <main className="original-app">
    ...
    </div></div>;const renderMenu=(interactive:boolean,eye:'left'|'right')=>cloneElement(menuElement,{className:`${menuElement.props.className} stereo-eye stereo-eye-${eye}`,'aria-hidden':interactive?undefined:true});const syncEyeScroll=(stage:HTMLDivElement|null)=>{if(!stage)return;const sync=(event:Event)=>{const target=event.target;if(!(target instanceof HTMLElement)||!target.classList.contains('original-menu'))return;stage.querySelectorAll('.original-menu').forEach(menu=>{if(menu!==target){menu.scrollTop=target.scrollTop;menu.scrollLeft=target.scrollLeft}})};stage.addEventListener('scroll',sync,true);return ()=>stage.removeEventListener('scroll',sync,true)};return stereo.mode==='off'?renderMenu(true,'left'):<div className="stereo-menu-stage" data-mode={stereo.mode} style={stereoStageStyle} ref={syncEyeScroll}>{renderMenu(true,'left')}{renderMenu(false,'right')}</div>})()}
