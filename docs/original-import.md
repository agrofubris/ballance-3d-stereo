...
BMap's PNG export writes every texture without alpha (`StbSaveFile` is called with `save_alpha=false`), so `prepare-original.py` re-extracts the alpha-bearing TGAs that the runtime samples from the game's `Textures` directory with ffmpeg: the dome and transformer shadow decals, `Laterne_Schatten`, `Laterne_Verlauf`, `Modul18_Gitter`, `Stick_Bottom` and `Column_beige_fade`. `Laterne_Verlauf`'s source alpha is uniformly zero and its level material is `GREATER` with `alphaRef` 1, so the original discards every pane fragment; preserving the zero alpha removes the opaque black panes the default conversion produced. Its MenuLevel material has alpha testing disabled and blends `ONE,ONE`, so its RGB remains visible additively through the recovered blend mapping. The menu button textures and `Font_1` also lose alpha in conversion, but the current interface renders browser text and CSS frames rather than those bitmaps, so they stay on the default path.

Blended materials map their recovered `sourceBlend`/`destBlend` factors explicitly instead of assuming Three's named modes. D3D8 applies one factor pair to every channel, while Three's named modes use separate alpha factors and are not equivalent (non-premultiplied `AdditiveBlending` is `SRC_ALPHA,ONE`, not `ONE,ONE`). The shipped pairs are `ONE,ONE`, `SRC_ALPHA,INV_SRC_ALPHA` and `ONE,SRCALPHA`, set with `CustomBlending`, `AddEquation` and matching alpha factors; packs or pairs without recovered blend fields keep Three's `NormalBlending`.
