...
export default function App() {
  ...
  return <main className="original-app">
    ...
    {!panel&&!busy&&(()=>{const hud=<OriginalHud points={points} lives={state.lives}/>;return stereo.mode==='off'?hud:<div className="stereo-menu-stage" data-mode={stereo.mode} style={stereoStageStyle}><div className="stereo-eye stereo-eye-left">{hud}</div><div className="stereo-eye stereo-eye-right" aria-hidden="true">{hud}</div></div>})()}
    ...
    </div></div>;const renderMenu=(interactive:boolean,eye:'left'|'right')=>cloneElement(menuElement,{className:`${menuElement.props.className} stereo-eye stereo-eye-${eye}`,'aria-hidden':interactive?undefined:true});return stereo.mode==='off'?renderMenu(true,'left'):<div className="stereo-menu-stage" data-mode={stereo.mode} style={stereoStageStyle}>{renderMenu(true,'left')}{renderMenu(false,'right')}</div>})()}
