...
import type {StereoSettings} from './stereo-renderer'
import {OriginalGamepad, type GamepadSettings} from '../controls/gamepad'
...
export class OriginalEngine {
  ...
  readonly stereo: StereoRenderer
  ...
  finish?: Trigger
  keys = new Set<string>()
  /** Shared input adapter used by gameplay and the React menu layer. */
  readonly gamepad = new OriginalGamepad()
  ...
  private cameraStickDirection = 0
  ...
  constructor(host: HTMLElement, onState: (state: GameState) => void, originalSolver=false) {
    ...
    this.stereo = new StereoRenderer(this.renderer)
  ...
  animate = (now: number) => {
    ...
    if (controller.connected) {
      this.touch.x = controller.moveX
      this.touch.z = controller.moveZ
      this.touch.brake = controller.pressed.has('overview')
      if (controller.justPressed.has('cameraLeft')) this.turnCamera('left')
      if (controller.justPressed.has('cameraRight')) this.turnCamera('right')
      if (controller.justPressed.has('pause') && this.state.phase === 'playing') this.pause()
      const cameraStick = this.gamepad.getSettings().cameraStick
      const direction = cameraStick && this.state.phase === 'playing' ? (controller.cameraX < -0.65 ? -1 : controller.cameraX > 0.65 ? 1 : 0) : 0
      if (!direction) this.cameraStickDirection = 0
      else if (direction !== this.cameraStickDirection) {
        this.cameraStickDirection = direction
        this.turnCamera(direction < 0 ? 'left' : 'right')
      }
    } else {
      this.touch.x = 0
      this.touch.z = 0
      this.touch.brake = false
      this.cameraStickDirection = 0
    }
  ...
  setStereo(settings: StereoSettings) { this.stereo.setSettings(settings) }
  setGamepadSettings(settings: GamepadSettings) { this.gamepad.setSettings(settings) }
